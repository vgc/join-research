...
function tryExtensionCut7(svgGroup, se0, se1, arguments) {
    if (se0.isExtension && se1.isCut) {
        ...
        if (se0.radius < se1.radius) {
            ...
            if (se1.t < se1.tCutMax) {
                ...
                const h = Math.sqrt(se1.tCutMax*se1.tCutMax + se1.radius*se1.radius);
                const tCutA = Math.sqrt(h*h - se0.radius*se0.radius);
                const coeff = se1.tCutMax / se1.t;
                p0 = se0.outlineOrigin;
                p1 = e1.outlinePoint(se1.side, se1.tCutMax);
                pX = e0.outlinePoint(se0.side, -tCutA * coeff * coeff);
            }
        }
        else {
...
var tryExtensionCut7Method = {
    name: "ExtensionCut7",
    prettyname: "ExtensionCut7",
    function: tryExtensionCut7,
    parameters: [
    ]
};
